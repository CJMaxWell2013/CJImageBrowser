...
{
    CJImageInfo *photo = nil;
    if (index > 0) photo = _photos[index - 1];
    if (index < _photos.count - 1) photo = _photos[index + 1];
    [[SDWebImageManager sharedManager] loadImageWithURL:photo.url options:SDWebImageLowPriority|SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
